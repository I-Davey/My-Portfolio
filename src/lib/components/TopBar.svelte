  ...
  <div class="top-bar-container">
    <header class="top-bar-navbar-interactive">
      <div class="logo-arrow-container">
        <a
          href="/{subpageName.toLowerCase()}"
          class="logo-arrow-container"
          on:click={() => {
            setPage(subpageName.toLowerCase());
          }}
          style="opacity: {isSubpage ? 1 : 0}; pointer-events: {isSubpage
            ? 'auto'
            : 'none'};"><ArrowLeft color="white" size={40} /></a
        >
        ...
        <nav class="top-bar-nav">
          ...
          {#each paths as path}
            <a
              ...
              class={path.toLowerCase() == folderName
                ? "top-bar-navlink-selected"
                : "top-bar-navlink"}
              on:click={() => {
